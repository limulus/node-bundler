...
var Installer = module.exports = function (targetPath, versionSelector, opts) {
        ...
        fs.createReadStream(srcFilePath)
          ...
          .pipe(lzma.createStream("autoDecoder"))
            ...
            .pipe(tar.Extract({ path: cacheDir }))
              .on("error", reject)
              .on("end", function () { return resolve(srcDir) })
      })
    })
  })
  ...
  this._extractedInstallDir = Promise.join(this._installerFileName, this._downloadCacheDirForVersion, this._installerFile, function (installerFileName, cacheDir, installerFilePath) {
    return new Promise(function (resolve, reject) {
      var installDirPath = path.resolve(cacheDir, installerFileName)
      fs.exists(path.resolve(installDirPath, "bin", "iojs"), function (isExtracted) {
        ...
        fs.createReadStream(installerFilePath)
          ...
          .pipe(zlib.createGunzip())
            ...
            .pipe(tar.Extract({ path: cacheDir }))
              .on("error", reject)
              .on("end", function () { return resolve(installDirPath) })
      })
    })
  })
}
...
Installer.prototype.install = function (installPath, cb) {
  Promise.join(this._version, this._srcDir, this._extractedInstallDir, function (version, srcDir, extractedInstallDir) {
